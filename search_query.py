...
__author__ = 'Alvaro Sanchez, based on original job by Tom Dickinson'
    ...
    def save_tweets(self, tweets):
        ...
        for tweet in tweets:
            ...
            if tweet['created_at'] is not None:
                ...
                with open("tweets_" + QUERY + "_out.txt", "a") as out:
                    out.write(json.dumps(tweet) + "\n")

                t = datetime.datetime.fromtimestamp((tweet['created_at']/1000))
                fmt = "%Y-%m-%d %H:%M:%S"
                log.info("%i [%s] - %s" % (self.counter, t.strftime(fmt), tweet['text'][:20]))
...
if __name__ == '__main__':
    ...
    QUERY_TEAM_1 = sys.argv[1] # hashtag, word...
    QUERY_TEAM_2 = sys.argv[2] # hashtag, word...
    QUERY_FROM = sys.argv[3] # "2018-04-29"
    QUERY_TO = sys.argv[4] # "2018-04-30"

    rate_delay_seconds = 0
    error_delay_seconds = 5

    # Example of using TwitterSlice
    select_tweets_since = datetime.datetime.strptime(QUERY_FROM, '%Y-%m-%d')
    select_tweets_until = datetime.datetime.strptime(QUERY_TO, '%Y-%m-%d')
    threads = 50

    QUERY = QUERY_TEAM_1
    twitSlice = TwitterSlicer(rate_delay_seconds, error_delay_seconds, select_tweets_since, select_tweets_until,
                              threads)
    twitSlice.search(QUERY)
    print("TwitterSlicer collected for team 1 %i" % twitSlice.counter)

    QUERY = QUERY_TEAM_2
    twitSlice = TwitterSlicer(rate_delay_seconds, error_delay_seconds, select_tweets_since, select_tweets_until,
                              threads)
    twitSlice.search(QUERY)
    print("TwitterSlicer collected for team 2 %i" % twitSlice.counter)
